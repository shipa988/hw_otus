...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", `configs\config.yaml`, "config file (default is $./config/config.yaml)")
...
func initConfig() {
	...
	if addr, ok := viper.Get("RABBIT_ADDR").(string); addr != "" && ok {
		cfg.Rabbit.Addr = addr
	}
}
