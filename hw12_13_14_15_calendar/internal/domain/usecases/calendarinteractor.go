...
type CalendarInteractor struct {
	...
	alerts entities.NotifyRepo
	logger Logger
}

func NewCalendar(eRepo entities.EventRepo, nRepo entities.NotifyRepo, logger Logger) *CalendarInteractor {
...
func (c CalendarInteractor) DeleteEvent(ctx context.Context, userID, eventID string) (string, error) {
	...
	if err := c.events.DeleteByID(ctx, userID, eventID); err != nil {
...
func (c CalendarInteractor) GetWeekEvents(ctx context.Context, date, userID string) ([]*entities.Event, error) {
	...
	events, err := c.events.GetForPeriod(ctx, userID, dt, dt.AddDate(0, 0, 7))
	if err != nil {
...
func (c CalendarInteractor) GetMonthEvents(ctx context.Context, date, userID string) ([]*entities.Event, error) {
	...
	events, err := c.events.GetForPeriod(ctx, userID, dt, dt.AddDate(0, 1, 0))
	if err != nil {
